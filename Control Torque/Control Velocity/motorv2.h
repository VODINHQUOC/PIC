...
void qADC_Init(void)
{
  ADCON0.ADCS1 = 1;  // T sample = 64Tosc
  ADCON0.ADCS0 = 0;
  ADCON0.CHS2 = 0;
  ADCON0.CHS1 = 0;    // Channel 0 AN0
  ADCON0.CHS0 = 0;
  ADCON0.GO_DONE = 0;  //Stop conversion
  ADCON0.ADON = 1;   // start adc

  PIR1.ADIF = 0;  // Clear ADC Interrupt Flag
  PIE1.ADIE = 1;  // ADC Interrupt Enable Bit
  INTCON.PEIE = 1;  // Peripherals Interrupt Enable Bit
  INTCON.GIE = 1;   // Global Interrupts Enable Bit
...
uint16_t ADC_Read(uint8_t ADC_channel)
{
    if(ADC_Channel <0 || ADC_Channel >7)    // Check Channel Number Validity
    { return 0;}
    ADCON0 &= 0b11000101; // Clear The Channel Selection Bits
    ADCON0 |= ADC_Channel<<3;     // Select The Required Channel (ANC)
